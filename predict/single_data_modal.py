...
def train_model():
    ...
    train_x= np.hstack([stock[:401],news[:401]])
    #y就是要被预测的价格
    train_y= pre_stock_data.iloc[:407,9:].values
    train_y = series_to_supervised(train_y, 6, 7)

    test_x= np.hstack([stock[401:],news[401:]])
    test_y = pre_stock_data.iloc[401:,9:].values
    ...
    plt.plot(index[401:496] ,np.squeeze(test_y,axis=1), label='test_y')
    plt.plot(index[408:], np.round(np.squeeze(predict[:, 1:2], axis=1), 1), label='predict_test')
